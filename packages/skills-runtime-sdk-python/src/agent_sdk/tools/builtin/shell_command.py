"""
内置工具：shell_command（Codex parity；Phase 5）。

对齐规格：
- `docs/specs/skills-runtime-sdk/docs/tools-exec-sessions.md`

说明：
- `shell_command` 是对 Codex 工具名的兼容层；内部复用 `shell_exec`。
- 通过 `/bin/sh -lc <command>` 执行，尽量贴近“命令行一行”的语义。
"""
...
class _ShellCommandArgs(BaseModel):
    """shell_command 输入参数（对齐 Codex：command string + workdir 等）。"""

    model_config = ConfigDict(extra="forbid")

    command: str = Field(min_length=1, description="完整命令行字符串")
...
SHELL_COMMAND_SPEC = ToolSpec(
    name="shell_command",
    description="执行一条本地命令（string 形式），并返回 stdout/stderr/exit_code（Codex parity）。",
    parameters={
        "type": "object",
        "properties": {
...
def shell_command(call: ToolCall, ctx: ToolExecutionContext) -> ToolResult:
    """
    执行 shell_command（内部复用 shell_exec）。
    ...
    try:
        args = _ShellCommandArgs.model_validate(call.args)
    except Exception as e:
        return ToolResult.error_payload(error_kind="validation", stderr=str(e))

    inner_args: Dict[str, Any] = {"argv": ["/bin/sh", "-lc", str(args.command)]}
