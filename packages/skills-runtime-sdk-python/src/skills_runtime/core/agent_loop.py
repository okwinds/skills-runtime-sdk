...
from skills_runtime.core.run_errors import MissingRequiredEnvVarError, RunError, RunErrorKind
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # 防御性兜底：namespace 未配置或 skill 不存在时降级为空列表，不中断 run。
                    resolved = []
                                            ...
                                            "tool_calls": [
                                                {
                                                    "call_id": c.call_id,
                                                    "name": c.name,
                                                    "arguments": safety_gate.sanitize_for_event(
                                                        c,
                        ...
                        if decision is None:
                            ...
                            if decision == "handoff_new_run":
                                ...
                                try:
                                    ...
                                    # 防御性兜底：compaction 过程可能抛出任意异常（IO/LLM/解析等）。
                                    _emit_event(
                                        AgentEvent(
                                            type="run_failed",
                                            timestamp=now_rfc3339(),
                ...
                if pending_tool_calls:
                    ...
                    for call in pending_tool_calls:
                        ...
                        decision = safety_gate.evaluate(call)
                        if decision.action == "deny":
                            denied_result = safety_gate.build_denied_result(call, decision)
                            _emit_event(
                                AgentEvent(
                                    type="tool_call_finished",
                                    timestamp=now_rfc3339(),
                            ...
                            history.append({"role": "tool", "tool_call_id": call.call_id, "content": denied_result.content})
                            continue
