...
def test_cli_runs_metrics_events_path_ok(tmp_path: Path, capsys) -> None:  # type: ignore[no-untyped-def]
    ...
    code = main(["runs", "metrics", "--workspace-root", str(tmp_path), "--events-path", str(events_path)])
    payload = _parse_last_json(capsys.readouterr().out)
    assert code == 0
